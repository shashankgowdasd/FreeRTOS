...
TaskProfiler RedLEDTskProfiler,GreenLEDTskProfiler,BlueLEDTskProfiler;
TaskHandle_t RedLedTskHndl,BlueLedTskHndl,GreenLedTskHndl;
...
int msg[1] = {'H'};
uint32_t BluePriority;
uint32_t isSuspend = 0;
...
void vRedLEDCOntrollerTask(void *pvParams)
{
	xSemaphoreGive(xBinarySemaphore);   // to avoid the deadlock condition so i am giving the semaphore
	while(1)
	{
